...
if ($Battle_Classes->Phase_Sys($Card_Id, $System)) {
	if ($Card_Id != null) {
		...
		if ($Card_Id["board_id"] == 1) {
			$Battle_Classes->_Battle[$Battle_Classes->_Enemy_Id]["Board"][$Card_Id["card"]]["def"] -= $ServerData["Damage_status"];
		} else {
			$Battle_Classes->_Battle[$Battle_Classes->_Player_Id]["Board"][$Card_Id["card"]]["def"] -= $ServerData["Damage_status"];
		}
		unset($ServerData["Damage_status"]);
